...
export default function ForgetPassword() {
    ...
    const schema=z.object({
        email:z.email('Please enter valid email').nonempty('Enter your email associated weith your account'),
    })
    ...
    async function sendOTP(vals:{email:string}) {
       ...
       try{
        ...
        if(res.ok){
            ...
            const payload=await res.json();
            throw new Error(`${payload.message || 'Something went wrong, please try agian later'}`);
        }
  ...
  return (
    <Slide direction="left">
      ...
      <header className="font-poppins text-start">
        ...
        <p className="text-slate-400 mt-1 text-md">An OTP Code will be sent directly to your email</p>
      </header>
   <div className="flex flex-col gap-2 mt-5">
       <label className="font-poppins font-bold " htmlFor="email">Email Address</label>
      <Input type='email' placeholder="Email Addresss" id="email" {...register('email')}/>
      {
        formState.errors.email && <span className="text-red-500">{formState.errors.email.message}</span>
      }
      <Button disabled={loading} className="cursor-pointer">{loading ? <i className="fa-solid fa-spinner fa-spin"></i>:'Send OTP'}</Button>
   </div>
