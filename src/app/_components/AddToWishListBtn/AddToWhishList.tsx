...
export default function AddToWhishListBtn({pid}:{pid:string}) {
  ...
  async function handleAdd() {
    ...
    try{
     ...
     if(payload.status==='success'){
      ...
      toast('Product Added Successfully to whislist' ,{position:'top-right',className:'!bg-emerald-500'});
     }else{
  ...
  async function handleDelete(){
   ...
   if(payload.status==='success'){
    ...
    localStorage.setItem('wishlist',JSON.stringify(payload.data))
    setAdded(false);
    toast(`Product removed from wishlist`,{position:'top-right',className:"!bg-emerald-500"})
   }else{
    throw new Error(`${payload}`)
   }
...
toast(`${err}`,{position:'top-right',className:"!bg-red-500"})
  ...
  if(added){
    ...
    >
     {loading ?  <i className="fa-solid fa-spinner fa-spin"></i>: <i className="fa-solid fa-heart text-lg"></i>}
    </button>
    )
  }
  return (
